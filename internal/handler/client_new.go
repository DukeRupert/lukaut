...
// renderFormErrorTempl renders the form with an error flash
func (h *ClientHandler) renderFormErrorTempl(
	...
	var err error
	if isEdit {
		err = clients.EditPage(data).Render(r.Context(), w)
	} else {
		err = clients.NewPage(data).Render(r.Context(), w)
	}
	if err != nil {
		h.logger.Error("failed to render client form error", "error", err)
		http.Error(w, "Internal Server Error", http.StatusInternalServerError)
	}
}
